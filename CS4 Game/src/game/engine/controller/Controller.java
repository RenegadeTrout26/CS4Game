...
public class Controller {
    ...
    private Image getCachedImage(String filename) {
        return imageCache.computeIfAbsent(filename, key ->
            new Image(getClass().getResource(key).toExternalForm())
        );
    }
   
    @FXML private TextArea playerTxt;
    @FXML private TextArea console;
    private int counter = 1;

    @FXML private ProgressBar playerEnergy;
    @FXML private ProgressBar oppEnergy;
    @FXML private TextArea oppTxt;
    ...
    @FXML private StackPane cards;
    @FXML private StackPane cardsDrawn;
    ...
    private StackPane createCell(int i) {
        ...
        if (c instanceof ConveyorBelt) {
            ...
            monsterCellImage.setFitWidth(50);
            monsterCellImage.setFitHeight(50);
           ...
           l2.setTranslateY(25);
            ...
            doorCellImage = new ImageView(
                getCachedImage(setDoorCellIcon(((DoorCell) c).getRole())));
            doorCellImage.setFitWidth(50);
            doorCellImage.setFitHeight(50);
        ...
        if (c instanceof DoorCell)
            sp.getChildren().addAll(r,doorCellImage, l, l2);
        else if (c instanceof MonsterCell)
    ...
    private String setDoorCellIcon(Role role) {
        if (role.equals(Role.LAUGHER))
            return "LaugherDoor.jpeg";
        return "ScarerDoor.jpeg";
    }
    ...
    private ImageView createSockImageView() {
        ImageView sockImage = new ImageView(getCachedImage("ContaminationSock.jpeg"));
    ...
    public void resetPos(int pos, Monster curr) {
        ImageView s = curr.equals(game.getPlayer()) ? player : opponent;
        cells[pos].getChildren().add(s);
    }

    public void clearOldPos(int ogPos, Monster curr) {
        ImageView s = curr.equals(game.getPlayer()) ? player : opponent;
        cells[ogPos].getChildren().remove(s);
    }
    ...
    public void updatePlayerUI(Monster player) {
        ...
        p.append("Player 1: ").append(player.getName())
         .append("\nEnergy: ").append(player.getEnergy())
         .append("\nOriginal Role: ").append(player.getOriginalRole())
         .append("\nPosition: ").append(player.getPosition())
         .append("\nMonster Type: ");
        if (player instanceof Dynamo)      p.append("Dynamo");
        if (player instanceof MultiTasker) {
            p.append("MultiTasker");
            if (((MultiTasker) player).getNormalSpeedTurns() != 0)
                p.append("\nPower Up ACTIVATED: \nNormal speed for: ")
                 .append(((MultiTasker) player).getNormalSpeedTurns()).append(" turns");
        }
        if (player instanceof Schemer) p.append("Schemer");
        if (player instanceof Dasher) {
            p.append("Dasher");
            if (((Dasher) player).getMomentumTurns() != 0)
                p.append("\nPower Up ACTIVATED: \nIncreased momentum for:  ")
                 .append(((Dasher) player).getMomentumTurns()).append(" turns");
        }
        if (player.isConfused())
            p.append("\nCurrent Role: ").append(player.getRole())
             .append("\nConfusion turns left: ").append(player.getConfusionTurns());
        if (player.isFrozen())    p.append("\nFrozen");
        if (player.isShielded())  p.append("\nShield");

        playerTxt.setText(p.toString());
        playerTxt.setEditable(false);

        double en = player.getEnergy();
        playerEnergy.setProgress(en == 0 ? 0 : en / Constants.WINNING_ENERGY);
    }

    public void updateOppUI(Monster opp) {
        StringBuilder s = new StringBuilder();
        s.append("Player 2: ").append(opp.getName())
         .append("\nEnergy: ").append(opp.getEnergy())
         .append("\nOriginal Role: ").append(opp.getOriginalRole())
         .append("\nPosition: ").append(opp.getPosition())
         .append("\nMonster Type: ");
        if (opp instanceof Dynamo)      s.append("Dynamo");
        if (opp instanceof MultiTasker) s.append("MultiTasker");
        if (opp instanceof Schemer)     s.append("Schemer");
        if (opp instanceof Dasher)      s.append("Dasher");
        if (opp.isConfused())
            s.append("\nCurrent Role: ").append(opp.getRole())
             .append("\nConfusion turns left: ").append(opp.getConfusionTurns());
        if (opp.isFrozen())    s.append("\nFrozen");
        if (opp.isShielded())  s.append("\nShield");

        oppTxt.setText(s.toString());
        oppTxt.setEditable(false);

        double en = opp.getEnergy();
        oppEnergy.setProgress(en == 0 ? 0 : en / Constants.WINNING_ENERGY);
    }
    ...
    private ImageView createCardBackView() {
        ImageView cardBack = new ImageView(getCachedImage("CardBack.jpeg"));
        cardBack.setFitWidth(134);
    ...
    private void createCardFrontView() {
        String image;
        String cardDesc = "" + Currcard.getDescription();
        switch (Currcard.getName()) {
            case "Position Swap":      image = "SwapperCard1.jpg";      break;
            case "Contamination Code": image = "StartOverCard1.jpg";    break;
            case "2319 Alert":         image = "StartOverCard1.jpg";    break;
            case "Small Snatcher":     image = "EnergyStealCard50.jpg"; break;
            case "Sneaky Thief":       image = "EnergyStealCard100.jpg";break;
            case "Mega Drain":         image = "EnergyStealCard150.jpg";break;
            case "Super Shield":       image = "ShieldCard.jpg";        break;
            case "Mind Scramble":      image = "ConfusionCard2.jpg";    break;
            case "Total Confusion":    image = "ConfusionCard3.jpg";    break;
        ...
        ImageView cardFront = new ImageView(getCachedImage(image));
        cardFront.setFitWidth(201);
        cardFront.setFitHeight(274);
        cardFront.setPreserveRatio(false);

        cardsDrawn.getChildren().addAll(cardFront, sp);
        cardsDrawn.setScaleX(1.3);
        cardsDrawn.setScaleY(1.2);
    }

    public void drawCardUI() {
        if (cards.getChildren().size() != 2) {
            Label l = (Label) cards.getChildren().remove(cards.getChildren().size() - 1);
            String s = "" + (Integer.parseInt(l.getText()) - 1);
    ...
    private void displayInvalidMoveException(String title, String message) {
        if (!isExceptionActive) {
            ...
            alertStage.setScene(new Scene(pane, 500, 100));
            alertStage.show();
        }
    }
    ...
    public boolean isExceptionActive() { return isExceptionActive; }
    public void setExceptionActive(boolean b) { this.isExceptionActive = b; }
    ...
    public void initializePlayerIcon(Role role) {
    	   if (player == null || player1 == null) {
    	        System.err.println("Player ImageViews not yet injected — skipping initializePlayerIcon");
    	        return;
    	    }
        String theImage;
        if (role.equals(Role.SCARER)) {
            switch (game.getPlayer().getName()) {
                case "James P. Sullivan":    theImage = "Sullivan.jpeg";       break;
                case "Randall Boggs":        theImage = "RandallOpp.jpeg";     break;
                case "Roz":                  theImage = "RozOpp.jpeg";         break;
                case "Henry J. Waternoose": theImage = "WaternooseOpp.jpeg";  break;
                default:                     theImage = "Sullivan.jpeg";       break;
            }
        } else {
            switch (game.getPlayer().getName()) {
                case "Mike Wazowski": theImage = "Wazowski.jpeg"; break;
                case "Celia Mae":     theImage = "Celia.jpeg";    break;
                case "Fungus":        theImage = "Fungus.jpeg";   break;
                case "Yeti":          theImage = "Yeti.jpeg";     break;
                default:              theImage = "Wazowski.jpeg"; break;
            }
        }
        Image playerImage = getCachedImage(theImage);
        player.setImage(playerImage);
        player1.setImage(playerImage);
    }

    public void initializeOppIcon(Role role) {
    	  if (opponent == null || player2 == null) {
    	        System.err.println("Opponent ImageViews not yet injected — skipping initializeOppIcon");
    	        return;
    	    }
        String theImage;
        if (role.equals(Role.SCARER)) {
        ...
        Image oppImage = getCachedImage(theImage);
    ...
    private void confusedMonsterImage(Monster monster) {
        ...
        switch (monster.getName()) {
            case "James P. Sullivan":    image = "SullivanPlayer.jpeg"; break;
            case "Randall Boggs":        image = "Randall.jpeg";        break;
            case "Roz":                  image = "Roz.jpeg";            break;
            case "Henry J. Waternoose": image = "Waternoose.jpeg";     break;
            case "Mike Wazowski":        image = "WazowskiOpp.jpeg";   break;
            case "Celia Mae":            image = "CeliaOpp.jpeg";       break;
            case "Fungus":               image = "FungusOpp.jpeg";      break;
            case "Yeti":                 image = "YetiOpp.jpeg";        break;
            default: return;
        }
        Image monsterImage = getCachedImage(image);
    ...
    private String setMonsterCellIcon(Monster m) {
        ...
        if (m.getRole().equals(Role.SCARER)) {
            switch (m.getName()) {
                case "James P. Sullivan":    theImage = "SullivanPlayer.jpeg"; break;
                case "Randall Boggs":        theImage = "Randall.jpeg";        break;
                case "Roz":                  theImage = "Roz.jpeg";            break;
                case "Henry J. Waternoose": theImage = "Waternoose.jpeg";     break;
                default:                     theImage = "SullivanPlayer.jpeg"; break;
            }
        } else {
            switch (m.getName()) {
                case "Mike Wazowski": theImage = "WazowskiOpp.jpeg"; break;
                case "Celia Mae":     theImage = "CeliaOpp.jpeg";    break;
                case "Fungus":        theImage = "FungusOpp.jpeg";   break;
                case "Yeti":          theImage = "YetiOpp.jpeg";     break;
                default:              theImage = "WazowskiOpp.jpeg"; break;
            }
        }
        return theImage;
    }
